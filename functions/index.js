...
exports.onTeamLeadChange = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    return admin
      ...
      .collection("users")
      .doc(newApproverID)
      .set({ isApprover: true }, { merge: true })
      .then(doc => {
        console.log("Team lead changed succesfully");
        admin
          .firestore()
          .collection("users")
          .doc(retiredApproverID)
          .set({ isApprover: false }, { merge: true });
      })
...
exports.getMyRequests = functions.https.onCall(async (queryData, context) => {
  ...
  await Promise.all(
    leaveRequestArray.map(async item => {
      await admin
        .firestore()
        .doc(item.leaveTypeRef.path)
        .get()
...
exports.getLeaveRequestDetail = functions.https.onCall(
  async (data, context) => {
    const documentId = data;
    await admin
      .firestore()
      .doc("/leaveRequests/{documentId}")
      .get()
      .then(async querySnapshot => {
        const leaveRequest = querySnapshot.data();
        await admin
          .firestore()
          .doc(leaveRequest.leaveTypeRef.path)
          .get()
          .then(documentSnapshot => {
...
exports.getTeamLeaves = functions.https.onCall(async (data, context) => {
  const userId = context.auth.uid;
  const user = await _getUser(userId);
  const leaveRequestArray = await _getLeaves(
    userId,
    _isAdmin(user),
    data.queryData
  );
  ...
  if (leaveRequestArray.length > 0) {
    await Promise.all(
      leaveRequestArray.data.map(async item => {
        // Retrieve leave types of the leave requests
        await admin
          .firestore()
          .doc(item.leaveTypeRef.path)
          .get()
...
const _getTeamLeaves = async (userid, queryData) => {
  ...
  await admin
    .firestore()
    .collection("teams")
    .where("leadUser", "==", userid)
    .get()
    .then(snapshot => {
      if (!snapshot.empty) {
        // If he/she is, get members of the team
  ...
  if (teamMembers.length > 0) {
    // If the team has at least one memeber, retrieve their request data
    let collection = admin
      .firestore()
      .collection("leaveRequests")
      .where("createdBy", "in", teamMembers);
    ...
    if (queryData.lastDocument != "end") {
      ...
      query.get().then(async querySnapshot => {
        ...
        for (const doc of querySnapshot.docs) {
          ...
          leaveDoc.id = doc.id;
          if (leaveDoc.leaveTypeRef) {
            await _getSpecificLeaveType(doc.data().leaveTypeRef.path).then(
              documentSnapShot => {
...
const _setDocumentField = (collection, doc, field, fieldValue) => {
  ...
  return new Promise((resolve, reject) => {
    admin
      ...
      .doc(doc)
      .set({ [field]: fieldValue }, { merge: true })
      .then(response => {
        resolve(response);
      })
      .catch(error => {
...
exports.changeLeaveStatus = functions.https.onCall(async (data, context) => {
  ...
  await _getLeaveRequestFromID(documentId)
    .then(async response => {
  ...
  await _getUser(leaveRequest.createdBy)
    .then(async response => {
  ...
  await _getLeaveTypeByRef(leaveRequest.leaveTypeRef.path)
    .then(async response => {
  ...
  await _setDocumentField("leaveRequests", documentId, "status", newStatus)
    .then(async response => {
  ...
  if (oldStatus === status.WAITING && newStatus === status.APPROVED) {
    if (leaveType.effectsTo == "Annual") {
      await _setDocumentField(
        ...
        "annualCredit",
        documentOwner.annualCredit - leaveRequest.duration
      )
        .then(async response => {
      ...
      await _setDocumentField(
        ...
        leaveRequest.createdBy,
        "excuseCredit",
        documentOwner.excuseCredit - leaveRequest.duration
      )
        .then(async response => {
